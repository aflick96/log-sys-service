...
public class UserService {
    ...
    public User registerUser(String email, String password) {
        if (ur.existsByEmail(email)) throw new RuntimeException("Email already exists: " + email);
    ...
    public boolean isTokenValid(String tokenValue) {
        if (tokenValue == null || tokenValue.isEmpty()) return false; // Invalid token value

        // Find the token by its value
        Optional<Token> optionalToken = tr.findByTokenValue(tokenValue);
        if (optionalToken.isEmpty()) return false; // Token not found
    ...
    public String getEmailFromToken(String tokenValue) {
        if (tokenValue == null || tokenValue.isEmpty()) return null; // Invalid token value

        // Find the token by its value
        Optional<Token> optionalToken = tr.findByTokenValue(tokenValue);
        if (optionalToken.isEmpty()) return null; // Token not found
    ...
    public String getTokenFromEmail(String email) {
        ...
        if (email == null || email.isEmpty()) return null; // Invalid email

        // Find the user by email
        Optional<User> optionalUser = ur.findByEmail(email);
        if (optionalUser.isEmpty()) return null; // User not found
